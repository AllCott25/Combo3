...
class WallpaperAnimation {
  constructor() {
    ...
    this.state = 'STATIC';
    this.staticStartTime = Date.now(); // Initialize with current time
    this.staticTimer = 15; // Reduced from 30 to 15 frames (0.5 seconds) - APlasker
    this.splitStartTime = null;
    this.closeStartTime = null;
    this.openStartTime = null;
    this.minimumStaticDuration = 500; // Reduced from 750ms to 500ms - APlasker
    this.splitDuration = 900; // Reduced from 1800ms to 900ms (0.9 seconds) - APlasker
    this.closeDuration = 600; // Reduced from 1200ms to 600ms (0.6 seconds) - APlasker
    this.openDuration = 900; // Reduced from 1800ms to 900ms (0.9 seconds) - APlasker
    
    // Split animation properties
    this.topHalfOffsetY = 0;
    this.bottomHalfOffsetY = 0;
    this.splitProgress = 0; // 0 to 1
    this.progress = 0; // General progress tracker - APlasker
  ...
  startSplitReveal() {
    if (this.state === 'STATIC') {
      ...
      if (timeInStatic >= this.minimumStaticDuration) {
        ...
        this.progress = 0; // Reset progress for split animation - APlasker
        this.splitProgress = 0; // Reset split progress - APlasker
      }
    }
  }
  ...
  startCookTransition() {
    if (this.state === 'READY_FOR_COOK') {
      ...
      this.progress = 1; // Start from fully open position - APlasker
  ...
  update() {
    // Mobile-specific timing adjustments - made faster for better responsiveness
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    const mobileSpeedMultiplier = isMobile ? 1.2 : 1.5; // Increased speed - was 0.8/1.0, now 1.2/1.5 - APlasker
    ...
    switch (this.state) {
      ...
      case 'SPLITTING':
        // Use frame-based progress instead of millisecond-based for more reliable transitions - APlasker
        this.progress += (1 / (this.splitDuration / 16.67)) * mobileSpeedMultiplier; // 16.67ms per frame at 60fps
        this.splitProgress = Math.min(this.progress, 1); // Cap at 1.0 - APlasker
        if (this.progress >= 1) {
          console.log("🎯 Split animation complete - transitioning to READY_FOR_COOK");
      ...
      case 'CLOSING':
        this.progress -= (1 / (this.closeDuration / 16.67)) * mobileSpeedMultiplier;
        this.splitProgress = Math.max(this.progress, 0); // Cap at 0.0 - APlasker
        if (this.progress <= 0) {
          console.log("🎯 Close animation complete - transitioning to OPENING");
          this.state = 'OPENING';
          this.progress = 0;
      ...
      case 'OPENING':
        this.progress += (1 / (this.openDuration / 16.67)) * mobileSpeedMultiplier;
        this.splitProgress = Math.min(this.progress, 1); // Cap at 1.0 - APlasker
        if (this.progress >= 1) {
          console.log("🎯 Open animation complete - transitioning to COMPLETE");
          this.state = 'COMPLETE';
          this.progress = 1;
          this.splitProgress = 1;
        }
  ...
  draw() {
    ...
    // Calculate split animation offsets based on progress - APlasker
    const maxOffset = 100; // Reduced from 200 to 100 pixels for faster, less dramatic animation - APlasker
    this.topHalfOffsetY = -this.splitProgress * maxOffset; // Top half moves up
    this.bottomHalfOffsetY = this.splitProgress * maxOffset; // Bottom half moves down
    ...
    console.log(`🎨 Drawing split wallpaper wall - State: ${this.state}, Progress: ${this.splitProgress.toFixed(2)}, Top offset: ${this.topHalfOffsetY.toFixed(1)}, Bottom offset: ${this.bottomHalfOffsetY.toFixed(1)}`);
  }
